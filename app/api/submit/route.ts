...
export async function POST(request: Request) {
  ...
  const scriptUrl = process.env.SUBMISSION_SCRIPT_URL;
  if (!scriptUrl) {
    console.error("SUBMISSION_SCRIPT_URL is not configured.");
    return NextResponse.json(
      {
        status: "error",
  ...
  try {
    ...
    try {
      ...
      console.error(
        `Submission script returned ${isHtml ? "an HTML error page" : "non-JSON"}. ${
          isHtml
            ? "Check the submission deployment (Execute as: Me, Access: Anyone) and that the sheet has a tab named exactly 'Submissions'."
    ...
    if (!upstream.ok || result?.status !== "success") {
      console.error(
        "Submission script returned failure:",
        result?.message ?? upstream.status,
      );
      return NextResponse.json(
        {
          status: "error",
